...
class CN2algorithm():
	...
	def __init__(self,train_data_csv):
		"""
		...
		#train,test = train_test_split(unsplit_data,test_size=0.33, random_state=42)
		...
		self.train = pd.read_csv(train_data_csv, names = cols, usecols=num_cols, header = 0)
	...
	def split_data(csv):	
		random_indices = permutation(csv.index)
		test_cutoff = math.floor(len(csv)/5)
		print(test_cutoff)
		test = csv.loc[random_indices[1:test_cutoff]]
		train = csv.loc[random_indices[test_cutoff:]]

		return test, train

	def fit_CN2_Model(self):
		"""
		...
		selectors = self.rule_creation()
		remaining_examples = self.train
		rule_list = []
		...
		while len(remaining_examples) >= 1:
			...
			while  best_new_calculate_significance > 0.5:
				...
				if len(rules_to_specialise) == 0:
					ordered_rule_results = self.apply_heuristics(selectors, remaining_examples)
					trimmed_rule_results = ordered_rule_results[0:self.max_star_size]
				elif len(rules_to_specialise) != 0: 		 
					specialised_rules = self.specialisation_process(rules_to_specialise,selectors)
					#import ipdb;ipdb.set_trace(context=8)
					ordered_rule_results = self.apply_heuristics(specialised_rules, remaining_examples)
					trimmed_rule_results = ordered_rule_results[0:self.max_star_size]
				...
				considered_results =  self.order_rule_list(considered_results).iloc[0:2]
	...
	def test_model(self, rule_list, data_set = 'default'):
		...
		for rule in rule_list:
			...
			#otherwise generate statistics about rule then save and remove examples from the data and test next rule.
				...
				class_of_covered_examples = rule_coverage_dataframe['PMMname']
				#import ipdb;ipdb.set_trace(context=8)
	...
	def apply_heuristics(self, complexes, data_set = 'default'):
		"""
		...
		for row in complexes:
			...
			#calculate stats for non 0 coverage rules
				...
				entropy_of_rule = self.calculate_entropy(rule_coverage)
				significance_of_rule = self.calculate_significance(rule_coverage)
				laplace_accuracy_of_rule = self.calculate_laplace_accuracy(rule_coverage)
				class_attrib = rule_coverage['PMMname']
				#import ipdb;ipdb.set_trace(context=8)
	...
	def order_rule_list(self, dataFrame_of_rules):
		"""
	...
	def rule_creation(self):
		...
		for att in attributes:
			possAttribVals[att] = set(self.train[att])
	...
	def attribute_value_pair(self,passed_complex):
		"""
		...
		attributes = self.train.columns.values.tolist() 	
		for att in attributes:
			rule[att] = list(set(self.train[att]))
	...
	def complex_coverage(self, passed_complex, data_set = 'default'):
		...
		rule = self.attribute_value_pair(passed_complex)
		if rule == False:
			return [],[]
	...
	def calculate_entropy(self,covered_data):
		...
		named class.
		""" 
		class_series = covered_data['PMMname']
	...
	def calculate_significance(self,covered_data):
		"""
		...
		"""
		covered_classes = covered_data['PMMname']
		...
		train_classes = self.train['PMMname']
...
lenseFit = CN2algorithm('SDLC1.csv')
lenseRules = lenseFit.fit_CN2_Model()
# lenseTest = lenseFit.test_model(lenseRules,lenseFit.test)[0]

...
class CN2algorithm():
        ...
        def __init__(self,df):
            ...
            self.df = pd.read_csv(df, names = cols, usecols=num_cols, header = 0)

            random_indices = permutation(self.df.index)
            test_cutoff = math.floor(len(self.df)/5)
            print(test_cutoff)
            self.test = self.df.loc[random_indices[1:test_cutoff]]
            self.train = self.df.loc[random_indices[test_cutoff:]]
        ...
        def fit_CN2(self):
                ...
                while len(remaining_examples) >= 1:
                    ...
                    rl = pd.DataFrame(rule_list) 
                    rl.to_csv('rule_list.csv', index=False)
...
if __name__ == '__main__':
	...
	"""
lenseFit = CN2algorithm('SDLC2.csv')
Rules = lenseFit.fit_CN2()
lenseTest = lenseFit.test_fitted_model(Rules,lenseFit.test)[0]    
